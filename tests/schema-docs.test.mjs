...
const fakeDocument = nodes => ({createElement(tag) {
  const n = {tag, dataset:{}, children:[], append(...c){this.children.push(...c)}, set innerHTML(v){throw Error('Unsafe HTML sink')}, set textContent(v){this.children=[v]}};
  nodes.push(n); return n;
}});
const flatten = node => typeof node === 'string' ? node : node.children.map(flatten).join('');

test('schema and source viewer URLs retain project and custom-domain base paths', () => {
  for (const base of ['https://example.org/nfcweb/index.html', 'https://example.org/index.html']) {
    const schemaUrl = new URL(viewerURL(id, base, '#/$defs/foo'));
    assert.equal(schemaUrl.pathname, new URL('schema.html', base).pathname);
    assert.equal(schemaUrl.searchParams.get('id'), id);
    assert.equal(schemaUrl.hash, '#/$defs/foo');
    const sourceUrl = new URL(sourceURL(id, base));
    assert.equal(sourceUrl.pathname, new URL('source.html', base).pathname);
    assert.equal(sourceUrl.searchParams.get('id'), id);
...
test('schema renderer only creates text nodes, never HTML from schema strings', () => {
  const nodes = [];
...
test('source tokenizer preserves indentation, escaping and scalar spellings exactly', () => {
  const source = '  "escaped\\"key": [null, true, false, -1.25e+3, "</script><img src=x>"]';
  const tokens = sourceTokens(source);
  assert.equal(tokens.map(token => token.text).join(''), source);
  assert.ok(tokens.some(token => token.type === 'key' && token.text === '"escaped\\"key"'));
  for (const type of ['null','boolean','number','string','punctuation']) assert.ok(tokens.some(token => token.type === type), type);
});
test('source renderer preserves original text and never interprets source as HTML', () => {
  const nodes = [];
  const document = fakeDocument(nodes);
  const source = '{\n\t"attack": "</span><img src=x onerror=alert(1)>",\n\t"n": 1,\n\t"ok": true,\n\t"none": null\n}\n';
  const rendered = renderSourceJSON(document, source);
  assert.equal(flatten(rendered), source);
  assert.ok(nodes.every(n => ['span', 'code'].includes(n.tag)));
  assert.ok(nodes.some(n => n.children.includes('"</span><img src=x onerror=alert(1)>"')));
});
...
test('artifact includes canonical and proposed schemas byte-for-byte with unchanged ids and provenance', async () => {
  ...
  for (const file of proposedFiles) {
    ...
    for (const base of ['https://example.org/nfcweb/index.html', 'https://example.org/index.html']) {
      const url = new URL(sourceURL(entry.id, base));
      assert.equal(url.pathname, new URL('source.html', base).pathname);
      assert.equal(url.searchParams.get('id'), entry.id);
    }
  }
  assert.deepEqual((await readdir('_site')).sort(), ['.nojekyll','index.html','schema-browser.js','schema-core.js','schema-manifest.json','schema.css','schema.html','schemas','source-browser.js','source.html'].sort());
});
test('overview and schema viewer expose source navigation; source viewer exposes provenance and status', async () => {
  ...
  assert.match(schemaBrowser, /View source/);
  assert.match(schemaBrowser, /sourceURL\(schema\.id, base\)/);
  assert.match(sourceBrowser, /Repository source path/);
  assert.match(sourceBrowser, /PROPOSED/);
